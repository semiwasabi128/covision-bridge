...
description: >
  深色科技氛圍 × 色彩質感 × 向量粒子 × 動態活潑 × 無限畫布連續感 × 可沉浸。
  六種設計屬性融合，核心靈魂：無限連續，不斷不跳。

> **📐 重要更新（2026-08-04）**
> 本檔 v0.x 已被正式拆分為兩層：
> - **本檔（DESIGN.md）**：保留 — 設計系統的原始靈魂與精神（永恆不變）
> - **[Bridge 統合設計語言 v0.1](docs/BRIDGE_UNIFIED_DESIGN_LANGUAGE.md)**：新裁決層 — 將設計來源蒸餾為 Bridge 的五種能力、三層介面與衝突裁決順序。
...
---
  # 基礎：深色科技 + 色彩質感
  canvas: "#07080a"          # 近黑藍 — 比 #1f2228 更深更冷，桌面技術工具感
  surface: "#101111"          # Surface 100 — 卡片、面板底色
  surface-elevated: "#1b1c1e" # 卡片表面 — badge、標籤
  surface-hover: "#252829"    # border 色 — hover 提升

  # 文字層級（white-on-dark + 灰階）
  text-primary: "#f9f9f9"     # near-white — 主文字
  text-secondary: "#cecece"   # 次要文字
  text-tertiary: "#9c9c9d"    # 灰 — 標籤、導航
  text-muted: "#6a6b6c"       # 暗灰 — disabled、佔位
  text-quaternary: "#434345"  # 最暗 — 非活躍

  # 互動色（語意色系 + 數據紫）
  accent-red: "#FF6363"       # Red — 品牌 punctuate、危險、錯誤
  accent-blue: "#55b3ff"      # 使用者 — 互動、連結、focus
  accent-green: "#5fc992"     # Green — 成功、連線
  accent-yellow: "#ffbc33"    # Yellow — 警告、注意
  accent-purple: "#533afd"    # Purple — 大腦/向量/數據可視化專用
  accent-navy: "#061b31"      # Navy — 深層數據文字、圖表標題
  accent-magenta: "#f96bee"   # Magenta — 漸層裝飾、粒子特效
  accent-ruby: "#ea2261"      # Ruby — 漸層裝飾、警示粒子
  accent-miro: "#5b76fe"      # 使用者 450 — 無限畫布互動、教學引導

  # 透明度疊層（opacity 系統 + 互動）
  border-subtle: "rgba(255,255,255,0.06)"   # 卡片邊界
  border-default: "rgba(255,255,255,0.10)"  # 標準邊框
  border-strong: "rgba(255,255,255,0.20)"   # 活躍/hover
  surface-glass: "rgba(255,255,255,0.05)"   # 微弱提升
  surface-glass-hover: "rgba(255,255,255,0.08)" # hover
...
typography:
  # 字體：monospace display + Inter body + 精準權重
  display:
...
rounded:
  ...
  sharp: 0px           # 邊框、技術標籤
  subtle: 4px          # badge
  standard: 8px        # 輸入框、小元件
  comfortable: 12px    # 標準卡片
  wide: 16px           # 大卡片、面板
  extra: 20px          # 展開面板
  pill: 50px           # 按鈕、tab
  circle: 999px        # 圓形圖示按鈕（CSS full-circle trick）
...
motion:
  ...
  transition-spring: "spring(mass: 1, stiffness: 280, damping: 24)"  # 彈簧 — 按鈕回饋
...
components:
  # 按鈕（pill + opacity transition + mono uppercase）
  button-primary:
...
Bridge 的設計語言融合多種設計傳統，核心靈魂是**無限連續**——使用者從打開 App 到關閉，
...
| 設計屬性 | 風格維度 | 具體取用 |
|------|---------|---------|
| **基礎氛圍** | 深色科技感 | monospace display 字體、銳利邊角、高對比白-on-dark、opacity 互動 |
| **色彩質感** | 暗底質感 | 近黑藍底色 `#07080a`、多層 macOS 陰影、正面 letter-spacing +0.2px、weight 500 baseline、opacity hover |
| **大腦/圖表** | 數據可視化 | 藍調陰影 `rgba(50,50,93)`、紫 `#533afd` 粒子、navy 深文、ruby/magenta 漸層 |
| **按鈕動態** | 活潑互動 | pill 50px 圓角、彩色漸層慶祝、variable font 精細權重、dashed focus ring |
| **轉場/教學** | 畫布連續 | 無限畫布連續感、ring shadow 邊框、spring 動畫、面板滑入不跳頁 |

## Colors

### 畫布層（深色科技 → 質感）

- **Canvas (`#07080a`)**：近黑藍。比 `#1f2228` 更深更冷，桌面技術工具的「內部空間」感。所有畫面的底層。
- **Surface (`#101111`)**：卡片、面板的底色。比 canvas 略亮一階，創造微弱浮起感。
- **Surface Elevated (`#1b1c1e`)**：badge、標籤、二級面板。

### 文字層級（白階 + 灰階）

- **Primary (`#f9f9f9`)**：主文字。近白，不純白以減少暗底刺眼。
...
- **畫布使用者 (`#5b76fe`)**：無限畫布互動、教學引導、連續轉場的視覺線索。
...
- **Geist Mono**：display 標題、按鈕文字、技術標籤。monospace-as-luxury 哲學——固定寬度在極端尺度下有節奏感。
- **Inter**：body、heading-m/s、caption。+0.2px letter-spacing + weight 500 baseline 在暗底上更易讀。
...
- **+0.2px tracking on dark**：獨門手法——暗底正面 letter-spacing 讓文字更透氣。
- **Weight 500 baseline**：body 用 500 不用 400，暗底更清晰。
- **Weight 300 for display**：輕量級大字 = 自信不喊叫。
...
| 圓角 | 用途 | 風格屬性 |
|------|------|------|
| 0px (sharp) | 技術標籤、代碼塊、數據表格邊框 | 基礎氛圍 |
| 4px (subtle) | badge、tag | 基礎氛圍/色彩質感 |
| 8px (standard) | 輸入框、小元件 | 色彩質感 |
| 12px (comfortable) | 標準卡片、訊息氣泡 | 色彩質感 |
| 16px (wide) | 大卡片、展開面板 | 轉場/教學 |
| 20px (extra) | 全螢幕面板、教學面板 | 轉場/教學 |
| 50px (pill) | 按鈕、tab | 按鈕動態 |
| 50% (circle) | 圓形圖示按鈕、頭像 | 按鈕動態 |
...
| 層級 | 手法 | 用途 | 風格屬性 |
|------|------|------|------|
| **Void** | 無陰影，canvas 底色 | 頁面背景 | 基礎氛圍 |
| **Ring** | 雙環陰影（outer + inset） | 卡片、面板 | 色彩質感 |
| **Floating** | 重陰影 + 白光暈 | 浮動面板、命令面板 | 色彩質感 |
| **Data** | 藍調多層陰影 | 大腦可視化、圖表 | 大腦/圖表 |
| **Glow** | 色彩光暈 | 粒子節點、活躍狀態 | 大腦/圖表 |

### 原則

- **不用單層 flat shadow**：陰影永遠成對（outer + inset）。
- **深度 = opacity 不是 shadow**：邊框透明度 0.06→0.10→0.20 創造層次。
- **數據用藍調陰影**：`rgba(50,50,93,0.25)` 讓陰影本身有品牌色。
...
- **互動色**：9 個（accentRed/使用者/Green/Yellow/Purple/Navy/Magenta/Ruby/accentMiro）
